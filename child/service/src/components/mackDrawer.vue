<template>
  <div class="markees">
    ...
    <div v-if="title === '测震数据' && this.markeArr.noiseLevel">
      <div class="nav">
        <div class="title">数据名称</div>
        <div class="navTil">
          {{ this.markeArr.staName }}({{ this.markeArr.staCode }})观测数据
        </div>
      </div>
      <div class="main">
        <div class="txtmain">
          <div class="txt">
            <span>测站代码: {{ this.markeArr.staCode }}</span>
          </div>
        </div>
        <div class="txtmain">
          <div class="header">
            <span> 站网名称： {{ this.markeArr.netName }} </span>
            <span> 台站级别： {{ this.markeArr.staLevel }} </span>
          </div>
          <!-- <div class="txt">
        ...
        >
          <div class="header">
            <span>观测环境：{{ this.markeArr.staArchitecture }}</span>
            <span> 建站时间： {{ new Date(this.markeArr.cTime).toLocaleDateString() }}</span>
          </div>
          <div class="txt">
            <span>井深：{{ this.markeArr.depth }}</span>
            <span>台基类型：{{ this.markeArr.staType }}</span>
         </div>
        </div>
        <div class="txtmain" style="border-top: 1px solid #000">
          ...
          <div v-if="this.markeArr.distributeUnit" class="txt">
            <span>联系单位：{{ this.markeArr.distributeUnit }}</span>
          </div>
          <div v-if="this.markeArr.distributeUnit" class="txt">
            <span>联系人：{{ this.markeArr.linkman }}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;联系方式：{{ this.markeArr.phone }}</span>
          </div>
          <div v-if="this.markeArr.distributeUnit" class="txt">
            <span>邮箱：{{ this.markeArr.email }}</span>
          </div>
        </div>
      </div>
    </div>
...
<!--              <span> 站网名称： {{ this.stationName != '黑龙江省' ? this.stationName.slice(0,2):'黑龙江' }} </span>-->
              <span>站网名称：国家站网</span>
              <span> 建站时间： {{ this.markeArr.cTime === undefined ? "暂无数据" : new Date(this.markeArr.cTime).toLocaleDateString()}}</span>
            </div>
            <div class="txt">
              <span>起始时间: ---</span>
              <span>终止时间: ---</span>
          ...
          >
            <div class="header">
              <span>仪器类型：{{JSON.parse(this.markeArr.staDescribe).yqlx}}</span>
            </div>
            <div class="txt">
              <span>采样率：100Hz</span>
              <span>数据格式: SEED</span>
...
<!--              <span>分发单位：{{ this.stationName }}地震局</span>-->
    ...
    <div v-if="title === '强震动数据'">
      <div class="nav">
        <div class="navTil">{{ this.markeArr.shockAddr }}{{ this.markeArr.shockLevel }}级地震信息</div>
      </div>
      <div class="main">
        <div class="txtmain">
          <!-- <div class="header">
          ...
          <div class="txt">
            <span>震级: {{ this.markeArr.shockLevel }}级</span>
            <span>地震深度: {{ this.markeArr.depth }}KM</span>
          </div>
        </div>
        <div class="txtmain">
          <div class="header">
            <span
              >发震日期: {{ this.markeArr.shockDate }}
              {{ this.markeArr.shockTime }}</span
            >
          </div>
          <!-- <div class="txt">
    ...
    <div v-if="title === '强震动地震台站数据信息'">
      <div class="nav">
        <div class="navTil">{{ this.markeArr.staName }}信息</div>
      </div>
      <div class="main">
        <div class="txtmain">
          <!-- <div class="header">
        ...
        <div class="txtmain">
          ...
          <div class="txt">
            <span>台站类型: {{ this.markeArr.staType }}</span>
            <span>台站编码：{{this.markeArr.staCode}}</span>
          </div>
...
<!--          <div class="txt">
    ...
    <div v-if="title === '地震周边台站'">
      <div class="nav">
        <div class="title">数据名称</div>
        <div class="navTil">
          {{ this.markeArr.sta_name }}{{ this.markeArr.sta_level }}台站
        </div>
      </div>
      <div class="main">
        <div class="txtmain">
          <div class="header">
            <span>
              {{ this.markeArr.sta_name
              }}{{ this.markeArr.sta_level }}台站</span
            >
          </div>
          <div class="txt">
            <span>台基类型: {{ this.markeArr.sta_type }}</span>
            <span>采样率: {{ this.markeArr.sampling_rate }}</span>
          </div>
        </div>
        <div class="txtmain">
          <div class="header">
            <span>仪器名称: {{ this.markeArr.instrument }}</span>
          </div>
          <div class="txt">
            <span>记录器型号: {{ this.markeArr.recorder_model }}</span>
            <span>传感器型号: {{ this.markeArr.sensor_model }}</span>
          </div>
        </div>
        <div class="txtmain">
          <div class="header">
            <span>数采类型:{{ this.markeArr.data_mining }} </span>
          </div>
    ...
    <div v-if="title === '科学台阵'">
      <div class="nav">
        <div class="title">数据名称</div>
        <div class="navTil">{{ this.markeArr.staName }}科学台阵数据
    ...
    <div v-if="title === '模拟资料'">
      <div class="nav">
        <div class="title">数据名称</div>
        <div class="navTil">{{ this.markeArr.staName }}模拟资料数据
