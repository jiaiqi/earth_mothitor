...
import { getDataStrong } from '@/api/vibration'
import {
  hotAdd,
  hotDataAdd
} from '@/api/hots'
  ...
  methods: {
    ...
    nodeClick(node) {
      ...
      if(node.name.toString().indexOf('-') != -1){
        ...
        // this.dayList = [
        //     {name: '20220825',leaf: false},
        //     {name: '20220724',leaf: false},
        //     {name: '20220623',leaf: false},
        //     {name: '20220522',leaf: false}
        //   ]
        //   // this.showTree = false
        // return resolve(this.dayList);
      ...
      if(node.name.toString().indexOf('-') != -1){
        ...
        // let par = 'staId='+this.days+'&yearDay='+date
        getDataStrong(encode({staId:this.days, yearDay: date.toString()})).then(res=>{
            let days = decode(res)
            days.forEach(item=>{
              item.dataDate = new Date(item.dataDate).toLocaleDateString().replaceAll('/','-'),
              item.leaf = true
    ...
    loadNode(node, resolve) {
      ...
      if (node.level === 1) {
          let year = []
          let par = 'staId='+this.days
          // this.yearList = [
          //   {name: '2022'+'年',leaf: false},
          //   {name: '2021'+'年',leaf: false},
          //   {name: '2020'+'年',leaf: false},
          //   {name: '2019'+'年',leaf: false}
          // ]
      ...
      if (node.level === 2) {
          let par = 'staId='+this.days+'&yearDay='+node.data.name
          // this.monthList = [
          //   {name: '202208',leaf: false},
          //   {name: '202207',leaf: false},
          //   {name: '202206',leaf: false},
          //   {name: '202205',leaf: false}
          // ]
      ...
      if (node.level === 3) {
        ...
        if (node.level === 1) {
          let year = []
          // let par = 'staId='+this.days
          getDataStrong(encode({staId:this.days})).then(res=>{
            if(!decode(res)[0]){
              this.$message.error('暂无数据')
              this.$emit('backSta')
              return
          ...
          let form = {
            keyName: '强震动数据-'+this.staName+'数据集',
      ...
      if (node.level === 2) {
          // let par = 'staId='+this.days+'&yearDay='+node.data.name
          getDataStrong(encode({staId:this.days, yearDay:node.data.name.toString()})).then(res=>{
            let days = decode(res)
            days.forEach(item=>{
              item.dataDate = new Date(item.dataDate).toLocaleDateString().replaceAll('/','-')
              item.leaf = true
