  ...
  export default {
    ...
    watch: {
      days(newVal) {
        ...
        this.dayList.forEach(item=>{
          item.dataDate = item.dataDate.replaceAll('-','')
        })
        this.getYearList()
        this.getMonthList()
      }
    ...
    methods: {
      ...
      getYearList() {
        ...
        let arr = []
        this.dayList.forEach(item=>{
          arr.push(item.dataDate.slice(0,4))
        })
        ...
        arr.forEach(item=>{
          ...
          if(this.yearList.length != 0){
            this.yearList.forEach(itm=>{
              if(itm.year == item){
                if(parseInt(itm.num) < parseInt(itm.yearDays)){
                  itm.num += 1
                }
                hasVal = true
              }
          ...
          temp.yearDays = this.getYearDays(item)
          if(!hasVal){
        ...
        //   }else {
        //     this.yearList.forEach((itm) => {
        //       if (item.dataDate.split('-')[0]+'年' == itm.year) {
        //         itm.num++;
        //       }
        //     })
        //   }
      ...
      getMonthList() {
        ...
        let arr = []
        this.dayList.forEach(item=>{
          arr.push(item.dataDate.slice(0,6))
        })
        ...
        arr.forEach(item=>{
          ...
          if(this.monthList.length != 0){
            this.monthList.forEach(itm=>{
              if(itm.month == item){
                if(parseInt(itm.num) < parseInt(itm.allDay)){
                  itm.num += 1
                }
                hasVal = true
              }
          ...
          temp.allDay = this.getDays(parseInt(item.slice(0,4)),parseInt(item.slice(4,6)))
          if(!hasVal){
