<template>
  <div class="app-container">
    <el-form
      ...
      v-show="showSearch"
    >
      <el-form-item label="url地址名称" prop="roleName">
        <el-input
    ...
    >
      ...
      <el-table-column label="url地址编号" prop="roleId" width="120" />
      <el-table-column
        label="url地址名称"
        prop="name"
...
import { listUrl } from '@/api/system/urlapi'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        roleId: [
          { required: true, message: "地址顺序不能为空", trigger: "blur" }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要"' + text + '""' + row.roleName + '"url地址吗？').then(function () {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除url地址编号为"' + roleIds + '"的数据项？').then(function () {
