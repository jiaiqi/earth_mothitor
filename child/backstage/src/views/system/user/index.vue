...
<script>
import { listUser, getUser, delUser, addUser, updateUser, resetUserPwd, changeUserStatus, getUserProfile } from "@/api/system/user"
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      //省局用户只能管理省局的，有“管理所有用户”的权限除外
      if(this.$store.state.user.permissions.includes('system:user:all') || this.$store.state.user.permissions[0] == '*:*:*'){
    ...
    getTreeselect() {
      ...
      //默认只能查看省局内用户
        ...
        getUserProfile().then(res=>{
          let deptId = res.data.deptId
          treeselect('?deptId='+deptId).then(response => {
            this.deptOptions = response.data
          })
        })
      }
