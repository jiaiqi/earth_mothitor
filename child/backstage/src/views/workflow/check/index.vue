<template>
  <div class="app-dataup">
    ...
    <!-- <el-form
      ...
      :inline="true"
    >
      <el-form-item label="提交用户" prop="roleName">
        <el-input
      ...
      >
        ...
        >
          ...
          <el-table-column
            ...
            width="150"
          />
          <el-table-column label="查看数据详情" width="100">
...
<script>
import { getAuditnode, getAuditmodelType } from '@/api/workflow/auditmodel'
...
export default {
  ...
  created() {
    ...
    getAuditmodelType(encode({})).then((res) => {
      list = decode(res.data)
      // 查询角色有权限的数据类
      let user = JSON.parse(getUser())
      let roleId = user.roles[0].roleId
      getAuditnode(encode({ roleid: roleId })).then((res) => {
        this.tabsList = decode(res.data)
        this.tabsList.forEach((item) => {
          item.modelName = list.filter((key) => key.val == item.modelId)[0].name
        })
      })
    })
  ...
  methods: {
    ...
    goReview(row) {
      this.$router.push({
        path: `/workflow/check-data/index/${row}`
      })
    },
    // 查看详情
    handleUpdate() {},
